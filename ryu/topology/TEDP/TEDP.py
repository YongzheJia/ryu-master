...
class ARPPATH_as_a_service(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        if (eth_typ == 0x0806):
            ...
            if (self.puerto_entrada_ok(id_switch_packet_in, inport, id_core,
                                       id_switch) == 1):  # si no tenemos ese puerto es no repetido
            ...
            # print "Core->"+str(id_core)+", id_switch_ant->"+str(id_switch_ant)+", id_switch_packet_in->"+str(id_switch_packet_in)
                ...
                if (len(self.puerto_confirmados[id_core][id_switch_packet_in]) == len(
                        self.mac_switches[id_switch_packet_in])):
                    # si ya hemos completado todos los puertos del switch debemos enviar un 0
                    match = parser.OFPMatch(eth_dst=dst, eth_type=2054, arp_sha=arp_sha, arp_tha=arp_sha)
                    actions = []
                    ##actions.append(parser.OFPActionSetField(arp_op=arp.ARP_REPLY)) #le indicamos que con el macth anterior modifique este dato
                    actions.append(
                        parser.OFPActionOutput(0))  # debemos cambiar la accion para que nos la mande al controler
    ...
    def crear_topologia(self, id_switch_A, id_switch_B, PORT_AB):
        ...
        if (len(self.topologia) == 0):
            ...
            if encontradoAB == 0:
                self.topologia.append(array_AB)
                print str(datetime.now() - self.time_convergence) + " us|" + str(self.packet_in) + "|" + str(
        ...
        if (self.arboles.has_key(id_switch_original)):  # si existe el arbol lo vamos actualizando
            ...
            if (not array_AB in self.arboles[id_switch_original]):
                ...
                for pos in range(0, len(self.arboles[id_switch_original])):
                    if (self.arboles[id_switch_original][pos][0] == id_switch_B and
                            self.arboles[id_switch_original][pos][1] == id_switch_A):  # si es inversa nos vale
    ...
    def install_initial_rules(self, id_core):
        for switch_id in self.switches.keys():
            ...
            ofproto = datapath.ofproto;
            # reamos el match
            match = parser.OFPMatch(eth_dst="FF:FF:FF:FF:FF:FF", eth_type=2054,
                                    arp_sha=str(self.mac_switches[id_core][0]),
                                    arp_tha=str(self.mac_switches[id_core][0]));
            ...
            if switch_id != id_core:  # solo enviamos estas reglas a los nodos no cores del arbol
                ...
                actions.append(parser.OFPActionSetField(eth_src=str(
                    self.mac_switches[switch_id][0])))  # le indicamos que con el macth anterior modifique este dato
    ...
    def crear_y_enviar_paquete(self, id_core):
        ...
        a = arp.arp(hwtype=1, proto=0x0800, hlen=6, plen=4, opcode=1,
                    src_mac=str(self.mac_switches[id_core][0]), src_ip='0.0.0.0',
                    dst_mac=str(self.mac_switches[id_core][0]), dst_ip='0.0.0.0')
        p = packet.Packet()
        p.add_protocol(e)
        p.add_protocol(a)
        p.serialize()
        ...
        out = parser.OFPPacketOut(datapath=self.switches[id_core],
                                  buffer_id=self.switches[id_core].ofproto.OFP_NO_BUFFER, in_port=0, actions=actions,
                                  data=p.data)
